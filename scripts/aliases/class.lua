...
function mtw.atkbal_alias(skill, what, target)
 if table.contains(mtw.skills, skill) then
  if target and (target ~= "") and (target ~= " ") then
   mtw.set_balance(what .. " ".. target)
  else
   mtw.set_balance(what .. " " .. mtw.target)
  end
 end
end

function mtw.dobal_alias(skill, what)
 if table.contains(mtw.skills, skill) then
  mtw.set_balance(what)
 end
end

function mtw.doeq_alias(skill, what)
 if table.contains(mtw.skills, skill) then
  mtw.set_equil(what)
 end
end

function mtw.atkpsi_alias(skill, what, target)
 if table.contains(mtw.skills, skill) then
  if target and (target ~= "") and (target ~= " ") then
   mtw.set_psi(what .. " " .. target)
  else
   mtw.set_psi(what .. " " .. mtw.target)
  end
 end
end

function mtw.dopsi_alias(skill, what)
 if table.contains(mtw.skills, skill) then
  mtw.set_psi(s)
 end 
end

function mtw.dooff_alias(skill, what)
 if table.contains(mtw.skills, skill) then
...
function mtw.charge_alias(matches)
 mtw.atkbal("Charge", "charge", matches[2])
end
...
function mtw.windsong_alias(matches)
 mtw.atkbal_alias("Windsong", "windflow", matches[2])
end
