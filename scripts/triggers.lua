...
function mtw.trigger_65(matches,multimatches)
mtw.fishing.stat = "waiting"
tempTimer( 1.8, [[send("fishing jerk", false)]] )
end

function mtw.trigger_66(matches,multimatches)
mtw.fishing.baited = false
mtw.fishing.stat = "reeling"
send("fishing reel", false)
end

function mtw.trigger_67(matches,multimatches)
mtw.fishing.stat = "baiting"
mtw.waiting.balance = false
end

function mtw.trigger_68(matches,multimatches)
mtw.fishing.baited = true
mtw.fishing.stat = "casting"
end

function mtw.trigger_69(matches,multimatches)
if mtw.fishing.fastfish and tonumber(matches[4]) > 10 then
 mtw.fishing.stat = "reeling"
else
...
function mtw.trigger_70(matches,multimatches)
tempTimer( 2.2, [[send("fishing tease", false)]] )
end

function mtw.trigger_71(matches,multimatches)
mtw.fishing.stat = "none"
echo("\nInvalid Direction! FDR to set one!")
end

function mtw.trigger_72(matches,multimatches)
if mtw.fishing.fish then
 send("unwield left", false)
 send("unwield right", false)
 send("wield pole", false)
end
end

function mtw.trigger_73(matches,multimatches)
mtw.toggles.fishing = false
echo("\nNo Bait!")
end

function mtw.trigger_74(matches,multimatches)
mtw.fishing.stat = "waiting"
mtw.reset_waiting()
mtw.do_core()
end

function mtw.trigger_75(matches,multimatches)
if mtw.fishing.baited then
 mtw.fishing.stat = "casting"
else
 mtw.fishing.stat = "baiting"
end
...
function mtw.trigger_76(matches,multimatches)
mtw.fishing.baited = true
end

function mtw.trigger_77(matches,multimatches)
mtw.fishing.baited = false
mtw.fishing.stat = "reeling"
mtw.reset_waiting()
if not mtw.fishing.fastfish and table.contains(mtw.skills, "Anachronize") and mtw.balance.anachronize and matches[2] == "great" then
 mtw.set_balance("cast anachronize")
end
mtw.do_fishing()
end

function mtw.trigger_78(matches,multimatches)
mtw.fishing.baited = false
mtw.fishing.stat = "baiting"
mtw.reset_waiting()
mtw.do_fishing()
...
function mtw.trigger_79(matches,multimatches)
mtw.fishing.baited = false
mtw.fishing.stat = "baiting"
end
...
mtw.atk_defend(matches[2], nil, "immolate", matches[4])
end
